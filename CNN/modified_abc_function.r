          ...
          trace = FALSE, maxit = 500, myweights, ...)
{
  ...
  sum1 <- 0
  for (j in 1:nss) {
    sum1 <- sum1 + ((scaled.sumstat[, j] - target[j])^2)*as.numeric(myweights[j])
  }
  dist <- sqrt(sum1)
  ...
  if (cond2 && !rejmethod) {
    ...
    adj.values=matrix(NA, nrow = (length(sumstat[, 1])*tol), ncol = numparam,
           byrow = T)
    return(list(adj.values=adj.values))
  }
    #stop_quietly()
  if (rejmethod) {
    if (cond2)
  ...
  list(adj.values=adj.values)
}

 normalise = function(vect1,vect2){
   media = mean(vect2);stand=sd(vect2)
   (vect1-media)/stand
 }
